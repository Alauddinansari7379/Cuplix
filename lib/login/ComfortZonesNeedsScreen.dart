...
class ComfortZonesNeedsScreen extends StatefulWidget {
  ...
  @override
  State<ComfortZonesNeedsScreen> createState() =>
      _ComfortZonesNeedsScreenState();
}
...
class _ComfortZonesNeedsScreenState extends State<ComfortZonesNeedsScreen> {
  ...
  // Text controllers
  final TextEditingController _emotionalNeedsController =
  ...
  // For formatting - make sure you have intl in pubspec.yaml: intl: ^0.17.0
  ...
  Future<void> _onContinue() async {
    ...
    if (_selectedComforts.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select at least one comfort zone.'),
        ),
      );
      return;
    }
    if (_emotionalNeedsController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Please describe what you need emotionally from your partner.',
          ),
        ),
      );
      return;
    ...
    if (_expectationsController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please describe what you expect from your partner.'),
        ),
      );
      return;
    }
    ...
    if (age < 13) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('You must be at least 13 years old. Current age: $age'),
        ),
      );
      return;
    }
    ...
    if (_togetherSince != null && _togetherSince!.isAfter(DateTime.now())) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Journey start date cannot be in the future.'),
        ),
      );
      return;
    }
    ...
    final payload = {
      ...
      // Backend-friendly ISO date strings:
      'dateOfBirth':
      ...
      'togetherSince':
          _togetherSince != null
              ? DateFormat('yyyy-MM-dd').format(_togetherSince!)
              : null,
  ...
  Widget build(BuildContext context) {
    ...
    // safe tile width calculation (responsive)
    final isWide = MediaQuery.of(context).size.width > 720;
    final tileWidth =
        isWide ? 120.0 : (MediaQuery.of(context).size.width - 160) / 2;
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'Where do you feel most comfortable? (Select all that apply)',
                ...
                Wrap(
                  ...
                  runSpacing: 12,
                  children:
                      _comfortChoices.map((c) {
                        final selected = _selectedComforts.contains(c.id);
                        return GestureDetector(
                          onTap:
                              () => setState(() {
                                if (selected)
                                  _selectedComforts.remove(c.id);
                                else
                                  _selectedComforts.add(c.id);
                              }),
                          child: Container(
                            width: tileWidth,
                ...
                const Text(
                  'What do you need emotionally from your partner?',
                  style: TextStyle(fontWeight: FontWeight.w600),
                ),
                const SizedBox(height: 8),
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText:
                        'Describe what makes you feel emotionally supported...',
                    filled: true,
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText:
                        'What behaviors or actions are important to you from your partner?',
                    filled: true,
                    fillColor: const Color(0xFFF8F6F8),
                    border: OutlineInputBorder(
                      borderRadius: BorderRadius.circular(12),
                        ...
                        Expanded(
                          child: Text(
                            _dob == null
                                ...
                                : _displayDateFormat.format(_dob!),
                            style: TextStyle(
                              color:
                                  _dob == null
                ...
                const SizedBox(height: 12),
                const Text(
                  'When did your journey of togetherness start?',
                  style: TextStyle(fontWeight: FontWeight.w600),
                ),
                const SizedBox(height: 8),
                GestureDetector(
                      ...
                      children: [
                        Expanded(
                          child: Text(
                            _togetherSince == null
                                ? 'dd-mm-yyyy'
                                : _displayDateFormat.format(_togetherSince!),
                          ),
                        ),
                                        ...
                                        : () {
                                          ...
                                          if (_selectedComforts.isEmpty) {
                                            ScaffoldMessenger.of(
                                              ...
                                              const SnackBar(
                                                content: Text(
                                                  'Select at least one comfort zone first',
                                                ),
                                              ),
                                            );
                                            return;
                                          }
                                          if (_emotionalNeedsController.text
                                                  .trim()
                                                  .isEmpty ||
                                            ...
                                            ScaffoldMessenger.of(
                                              ...
                                              const SnackBar(
                                                content: Text(
                                                  'Please fill the emotional needs and expectations fields',
                                                ),
                                              ),
                                            );
                                            return;
